        ...
        <section id="resources" className="py-24 bg-white">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-16">
                    <h2 className="font-serif text-3xl font-bold text-slate-900 mb-4">{t('KBTitle')}</h2>
                    <p className="text-slate-500">{t('KBSubtitle')}</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {categories.map((cat, idx) => (
                        <div key={idx} onClick={() => alert(`Accessing ${cat.title} archive...`)} className="group p-6 rounded-2xl border border-slate-100 bg-slate-50 hover:bg-white hover:shadow-xl hover:border-emerald-saudi/20 transition-all duration-300 cursor-pointer flex items-center justify-between">
                            ...
                            <div className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center text-slate-400 group-hover:border-gold-saudi group-hover:text-gold-saudi transition-all">
                                <ArrowRight size={18} />
                            </div>
                        </div>
                    ))}
